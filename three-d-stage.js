 ...
 * There is no on-screen toolbar: exporting is an occasional deliberate act,
 * so it is an API call rather than furniture parked over the scene. FBX
 * cannot be exported in the browser; GLB is the interchange format every
 * modern 3D tool imports.
 *
...
(() => {
  ...
  class ThreeDStage extends HTMLElement {
    constructor() {
      ...
      // No export toolbar: the download buttons sat over the scene the whole
      // time to serve a step taken once, if ever. Exporting is still here —
      // call stage.export('obj') or stage.export('glb') — just not on screen.
    ...
    async export(format) {
      ...
      await this._runExport(format === 'obj' ? 'obj' : 'glb');
      return true;
    }
     ...
     *  before. The no-object early return is not an attempt — export() was
     *  called before a model was set — and reports nothing. */
